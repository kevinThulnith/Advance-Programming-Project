...
const RoomDetailPage = () => {
  ...
  const totalCost = room ? nightsCount * room.pricePerNight : 0;
  ...
  if (loading) {
    return (
      <div className="flex flex-col min-h-screen bg-[var(--color-surface)]">
        <Navbar />
        <main className="flex-1 w-full flex items-center justify-center pt-24 pb-16">
          <div className="animate-pulse text-[var(--color-accent-gold)]">Loading room details...</div>
        </main>
        <Footer />
      </div>
    );
  ...
  if (error || !room) {
    return (
      <div className="flex flex-col min-h-screen bg-[var(--color-surface)]">
        <Navbar />
        <main className="flex-1 w-full flex items-center justify-center pt-24 pb-16">
          <div className="text-[#E53E3E] bg-red-50 p-6 rounded-lg border border-red-200">{error || 'Room not found.'}</div>
        </main>
        <Footer />
      </div>
    );
  ...
  // Backend only returns one imageUrl, so we create a mock array of 4 for the gallery UI
  const images = [
    room.imageUrl,
  ...
  // Hardcoded amenities for UI since backend doesn't store them
  const amenities = [
          ...
          <div className="w-full lg:w-1/3 fade-up" style={{animationDelay: '200ms'}}>
            <div className="glass-panel-light p-8 sticky top-32 shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-white/50 bg-white/60">
              ...
              <div className="mb-6 pb-6 border-b border-gray-200 flex items-baseline justify-between">
                <div>
                  <span className="text-4xl font-['Playfair_Display'] font-bold text-[var(--color-accent-gold)] drop-shadow-sm">${room.pricePerNight}</span>
              ...
              {nightsCount > 0 && (
                <div className="mb-6 bg-[rgba(201,168,76,0.05)] rounded-lg p-4 border border-[rgba(201,168,76,0.2)] animate-fade-in">
                  <div className="flex justify-between text-sm text-gray-600 mb-2">
                    ...
                    <span>${room.pricePerNight * nightsCount}</span>
                  </div>
